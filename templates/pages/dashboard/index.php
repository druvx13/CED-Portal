                    ...
                    <?php foreach ($programs as $p): ?>
                        <article class="code-snippet">
                            <header>
                                ...
                                <span class="badge"><?= \App\Core\View::h($p['language_name']) ?></span>
                            </header>
                            <pre><code class="hljs language-<?= \App\Core\View::h($p['language_slug']) ?>"><?php
                                $preview = mb_substr($p['code'], 0, 260, 'UTF-8');
                                echo \App\Core\View::h($preview . (mb_strlen($p['code'], 'UTF-8') > 260 ? "...\n" : ""));
                            ?></code></pre>
                            <div class="code-snippet-meta">
                                <span class="muted">By <?= \App\Core\View::h($p['username'] ?? 'Unknown') ?></span>
                                <span class="muted"><?= \App\Core\View::h($p['created_at']) ?></span>
                            </div>
                        </article>
                    <?php endforeach; ?>
                </div>
